            ...
            <form method="POST" action="{{ route('usuarios.update', $usuario) }}" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select
                        ...
                        class="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-4 py-2 transition focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                        <option value="Administrador" {{ old('cargo', $usuario->empleado->cargo ?? '') == 'Administrador' ? 'selected' : '' }}>Administrador</option>
                        <option value="Gerente/RRHH" {{ old('cargo', $usuario->empleado->cargo ?? '') == 'Gerente/RRHH' ? 'selected' : '' }}>Gerente/RRHH</option>
                        <option value="Asistente de RRHH" {{ old('cargo', $usuario->empleado->cargo ?? '') == 'Asistente de RRHH' ? 'selected' : '' }}>Asistente de RRHH</option>
                        <option value="Empleado" {{ old('cargo', $usuario->empleado->cargo ?? '') == 'Empleado' ? 'selected' : '' }}>Empleado</option>
                    </select>
                </div>
                {{-- Botón de envío --}}
