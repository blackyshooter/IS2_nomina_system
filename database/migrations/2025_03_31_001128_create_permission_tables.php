...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Modificar la tabla roles existente
        if (!Schema::hasTable('roles')) {
            Schema::create($tableNames['roles'], static function (Blueprint $table) {
                $table->bigIncrements('id_rol'); // Clave primaria
                $table->string('nombre_rol', 25); // Nombre del rol
                $table->string('descripcion', 25); // Descripción
                $table->unsignedBigInteger('id_usuario'); // Relación con usuarios
            });
        }

        // Crear las otras tablas relacionadas
        ...
        Schema::create($tableNames['model_has_roles'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole) {
            ...
            $table->foreign($pivotRole)
                ->references('id_rol') // Relación con id_rol de tu tabla roles
                ->on('roles')
                ->onDelete('cascade');
